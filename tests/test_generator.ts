...
async function call() {
  const question = await generator.getQuestionForSuggestion(document, suggestion)
  ...
  new Promise(resolve => rl.question("Q: " + question + "\nA: ", ans => {
    ...
    const updated = await generator.updateDocumentWithAnswer(document, question, answer as string)
    console.log(updated)
  })

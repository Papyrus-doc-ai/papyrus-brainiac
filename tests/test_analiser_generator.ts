...
import {Generator} from "../src/clients/generator.js";
...
async function call() {
  ...
  const question = await generator.getQuestionForSuggestion(document, response[0])
  ...
  new Promise(resolve => rl.question("Q: " + question + "\nA: ", ans => {
    ...
    const updated = await generator.updateDocumentWithAnswer(document, suggestion, answer as string)
    console.log(updated)
  })
